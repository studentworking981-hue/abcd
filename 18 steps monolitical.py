...
def run_pbf_defect_pipeline(gray_u8, params):
    """Runs imported PBF Steps 5-10 (Illumination/Denoise -> Multi-Scale DoG
    -> Sensitivity & Watershed -> Shape Filtering -> Full-Extent Band &
    Continuity -> Minute-Defect Rescue) on a single grayscale image, honoring
    each stage's own 'enabled' flag as its Skip toggle.

    Returns a dict with a visualization image for each of the 6 steps plus
    'gray_for_markers': the grayscale array to be handed to (what is now)
    Step 11 -- Histogram Markers. If every one of Steps 5-10 is skipped /
    detects nothing, gray_for_markers is IDENTICAL to the input gray_u8, so
    the rest of the pipeline (Steps 11-18) behaves exactly as before this
    integration (multiple-consecutive-skip = pure pass-through).
    """
    ...
    # Degenerate input (empty / fully-black ROI, e.g. before an ROI has
    # been selected) -- nothing meaningful to denoise or detect. Skip the
    # whole PBF pipeline and pass the image through untouched rather than
    # letting near-zero local variance flood the adaptive thresholds and
    # flag the entire frame as defective.
    if gray_u8.max() <= 3:
        zeros = np.zeros(gray_u8.shape, np.uint8)
        return dict(
            denoised_u8=gray_u8.copy(), dog_viz=gray_u8.copy(), sens_viz=zeros.copy(),
            shape_viz=zeros.copy(), extent_viz=zeros.copy(),
            final_overlay=cv2.cvtColor(gray_u8, cv2.COLOR_GRAY2BGR),
            final_mask=zeros.copy(), bright_mask=zeros.copy(), dark_mask=zeros.copy(),
    ...
    if p5['enabled']:
        background = _pbf_estimate_background(gray_f, p5['bg_sigma'])
        normalized = _pbf_flatten_illumination(gray_f, background)
        denoised = cv2.bilateralFilter(
            normalized.astype(np.uint8), int(p5['bilat_d']), float(p5['bilat_sc']), float(p5['bilat_ss'])
        ).astype(np.float32)
    else:
        normalized = gray_f.copy()
        denoised = gray_f.copy()
    denoised_u8 = np.clip(denoised, 0, 255).astype(np.uint8)

    # ROI: exclude black padding (matches pbf_pipeline's own convention);
    # this app's own Step 1 ROI mask has already blacked out non-ROI pixels.
    roi = (gray_u8 > 3).astype(np.uint8) * 255
    if roi.max() > 0:
        roi = cv2.erode(roi, np.ones((5, 5), np.uint8))
    else:
        roi = np.full(gray_u8.shape, 255, np.uint8)

    # ---- Step 6: Multi-Scale DoG ----
    if p6['enabled']:
        dog_fine = _pbf_difference_of_gaussians(denoised, p6['fine_sigma1'], p6['fine_sigma2'])
        dog_coarse = _pbf_difference_of_gaussians(denoised, p6['coarse_sigma1'], p6['coarse_sigma2'])
    else:
        dog_fine = np.zeros_like(denoised)
        dog_coarse = np.zeros_like(denoised)
    recon_radius = int(p6.get('recon_radius', 1)) if p6['enabled'] else 0
    dog_combined = dog_fine + dog_coarse

    roi_vals = denoised[roi > 0]
    if roi_vals.size == 0:
        roi_vals = denoised.reshape(-1)
    _, base_sigma = _pbf_robust_mu_sigma(roi_vals)

    sens_mask_viz = np.zeros(gray_u8.shape, np.uint8)
    shape_mask_viz = np.zeros(gray_u8.shape, np.uint8)
    extent_mask_viz = np.zeros(gray_u8.shape, np.uint8)
    ...
    for polarity, sign in (("bright", 1.0), ("dark", -1.0)):
        ...
        for name, dog_band, k, min_comp_area in (
            ("fine", dog_fine, p7['noise_k_fine'], p7['min_component_area_fine']),
            ("coarse", dog_coarse, p7['noise_k_coarse'], p7['min_component_area_coarse']),
        ):
            channel = np.clip(sign * dog_band, 0, None)
            if recon_radius > 0:
                channel = _pbf_opening_by_reconstruction(channel, recon_radius)

            if p7['enabled']:
                thr = _pbf_adaptive_threshold(channel, roi, k, robust=False)
                binary = ((channel > thr) & (roi > 0)).astype(np.uint8) * 255
                binary = _pbf_remove_small_components(binary, min_comp_area)
            else:
                # Skip = pass-through: no sensitivity threshold/area filter,
                # any positive band response is a raw candidate.
                binary = ((channel > 0) & (roi > 0)).astype(np.uint8) * 255

            close_r = p9['close_radius_fine'] if name == 'fine' else p9['close_radius_coarse']
            if p9['enabled'] and close_r > 0:
                binary = _pbf_close_and_fill(binary, close_r)

            if binary.max() == 0:
                seg = np.zeros(binary.shape, dtype=np.int32)
            elif p7['enabled']:
                markers, _ = _pbf_build_markers(binary, p7['local_max_min_dist'])
                surface = -channel
                seg = _pbf_watershed(surface, markers, mask=binary, compactness=p7['compactness'])
            else:
                # Skip = pass-through: plain connected components, no
                # watershed splitting of touching blobs.
                _, seg = cv2.connectedComponents(binary)

            pre_shape_masks[name] = (seg > 0).astype(np.uint8) * 255

            if p8['enabled']:
                band_mask, _ = _pbf_shape_filter(
                    seg, p8['min_area'], p8['max_area'], p8['min_circularity'],
                    p8['min_solidity'], p8['max_aspect_ratio'], p8['min_convexity'])
            else:
                # Skip = pass-through: keep every watershed/CC region as-is.
                band_mask = (seg > 0).astype(np.uint8) * 255
            band_masks[name] = band_mask

        sens_mask_viz = cv2.bitwise_or(sens_mask_viz, pre_shape_masks['fine'])
        sens_mask_viz = cv2.bitwise_or(sens_mask_viz, pre_shape_masks['coarse'])
        shape_mask_viz = cv2.bitwise_or(shape_mask_viz, band_masks['fine'])
        shape_mask_viz = cv2.bitwise_or(shape_mask_viz, band_masks['coarse'])

        # ---- Step 9: Full-Extent Band & Continuity ----
        if p9['enabled']:
            extent_depth = p9['extent_k'] * base_sigma
            extent_channel = _pbf_hdome_response(sign * denoised, extent_depth)
            ext_binary = ((extent_channel > 0.0) & (roi > 0)).astype(np.uint8) * 255
            ext_binary = _pbf_remove_small_components(ext_binary, p9['min_component_area_extent'])
            if p9['close_radius_extent'] > 0:
                ext_binary = _pbf_close_and_fill(ext_binary, p9['close_radius_extent'])
            if ext_binary.max() == 0:
                extent_mask = np.zeros(ext_binary.shape, np.uint8)
            else:
                markers, _ = _pbf_build_markers(ext_binary, p9['local_max_min_dist_extent'])
                surface = -extent_channel
                seg_ext = _pbf_watershed(surface, markers, mask=ext_binary, compactness=0.0)
                if p8['enabled']:
                    extent_mask, _ = _pbf_shape_filter(
                        seg_ext, p8['min_area'], p8['max_area'], p8['min_circularity'],
                        p8['min_solidity'], p8['max_aspect_ratio'], p8['min_convexity'])
                else:
                    extent_mask = (seg_ext > 0).astype(np.uint8) * 255
        else:
            extent_mask = np.zeros(gray_u8.shape, np.uint8)
        extent_mask_viz = cv2.bitwise_or(extent_mask_viz, extent_mask)

        combined_mask = cv2.bitwise_or(band_masks['fine'], band_masks['coarse'])
        combined_mask = cv2.bitwise_or(combined_mask, extent_mask)

        # ---- Step 10: Minute-Defect Rescue ----
        if p10['enabled']:
            combined_mask = _pbf_local_contrast_rescue(
                combined_mask, denoised, roi,
                ring_px=p10['micro_rescue_ring_px'], min_z=p10['micro_rescue_min_z'],
                max_seed_area=p10['micro_rescue_max_area'])
            relaxed_channel = np.clip(sign * dog_fine, 0, None)
            relaxed_thr = _pbf_adaptive_threshold(relaxed_channel, roi, max(1.0, p7['noise_k_fine'] - 1.5))
            relaxed_binary = ((relaxed_channel > relaxed_thr) & (roi > 0)).astype(np.uint8) * 255
            relaxed_binary = _pbf_remove_small_components(relaxed_binary, max(1, p7['min_component_area_fine'] // 2))
            rescued = _pbf_local_contrast_rescue(
                relaxed_binary, denoised, roi,
                ring_px=p10['micro_rescue_ring_px'], min_z=p10['micro_rescue_min_z'] + 0.5,
                max_seed_area=p10['micro_rescue_max_area'])
            combined_mask = cv2.bitwise_or(combined_mask, rescued)

        results[polarity] = combined_mask

    final_mask = cv2.bitwise_or(results["bright"], results["dark"])

    # Carried-forward grayscale for Step 11 onward: the denoised base image
    # (identity = original gray_u8 whenever Step 5 is also skipped), with
    # any detected-defect pixels forced to max intensity so the existing
    # Step 11 high/low threshold markers logic picks them up as foreground.
    # When Steps 6-10 detect nothing (all skipped, or tuned to find no
    # defects), final_mask is all-zero and gray_for_markers == denoised_u8
    # exactly -- i.e. an exact pass-through of Step 5's output, and (if
    # Step 5 is also skipped) an exact pass-through of the original gray.
    gray_for_markers = denoised_u8.copy()
    gray_for_markers[final_mask > 0] = 255

    dog_viz = cv2.normalize(np.abs(dog_combined), None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8) if p6['enabled'] else denoised_u8.copy()

    final_overlay = cv2.cvtColor(denoised_u8, cv2.COLOR_GRAY2BGR)
    final_overlay[results["bright"] > 0] = (0, 0, 255)   # red = bright defects (BGR)
    final_overlay[results["dark"] > 0] = (0, 140, 255)   # orange = dark defects (BGR)
...
"""
    ...
    def process_all_steps(self):
        ...
        # ================================================================
        pbf = run_pbf_defect_pipeline(gray, self.params)
